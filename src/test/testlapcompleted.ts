...
let currentOffsetMeters = START_OFFSET_METERS;

function getFakeGpsPosition(): { lat: number; lng: number } {
  const latOffset = metersToLat(currentOffsetMeters);
  const lat = FLAG_LAT + latOffset;
  const lng = FLAG_LNG;

  currentOffsetMeters += STEP_SIZE_METERS;

  if (currentOffsetMeters > END_OFFSET_METERS) {
    currentOffsetMeters = START_OFFSET_METERS;
  }
...
export function generateFakeTelemetryData(): ITelemetryData {
  ...
  return {
    ...
    Telemetry: {
      ...
      // GpsLatitude: 37.004658,
      // GpsLongitude: -86.36301354,
